...
previous_transmission_condition = 0;
compensate=0;
update_sliding_windows=5;
%load("trained_cls.mat");
model = encode_model(tgacChannel.DelayProfile);    
%% Processing Chain
...
for numPkt = 1:numPackets 
    ...
    if numPkt ==2
        SNR_pred=snrMeasured(1,2)/2+snrMeasured(1,1)/2;
    end   
    if numPkt>2
        SNR_pred=snrMeasured(1,numPkt-2)/6+snrMeasured(1,numPkt-1)/3+0.25*snrMeasured(1,numPkt)/2;   
    end
